...
# плотность воды, кг/м3 (используется в psi формулы V.5)
WATER_DENSITY = 1000.0

# рабочая скорость = 0.8 * скорость захлёбывания.
ADMISSIBLE_VELOCITY_FACTOR = 0.8

# Книга, стр. 158: "для эффективной и устойчивой работы колонны необходимо,
# чтобы её рабочие нагрузки были примерно на 20% ниже точки захлёбывания" -
# т.е. w_раб = 0.8 * w_захл. Значение подтверждено текстом книги.

# коэффициент избытка флегмы beta = R/R_мин (обозначение книги, стр. 74, см. (II.109))
BETA = 1.15
# ВЭТТ h_экв для колец Паля, м (формула и значение не менялись по просьбе пользователя)
H_EKV = 0.60

# --- коэффициенты пересчёта единиц измерения (только для перевода
#     "инженерных" исходных данных в СИ во входном блоке) ---
HOUR_TO_SEC = 3600.0        # 1 ч = 3600 с
# 1 сантипуаз (сПз) = 1e-3 Па*с (используется только
CP_TO_PAS = 1.0e-3
# внутри формулы V.5, см. пояснение там же)

# ============================================================
# ГРАФИК V-4: коэффициент k для уравнения (V.5)
# Александров, 1971, рис. V-4, стр. 159
# ============================================================
...
# ============================================================
# ГРАФИК II-14 (Джиллиленд): число теоретических тарелок N
# Александров, 1971, рис. II-14, стр. 74
...
# ============================================================
# СТРУКТУРЫ ДАННЫХ (все величины - в СИ)
# ============================================================
...
class Mixture:
    ...
    xF: list[float]         # состав питания, мол. доли
    alpha: list[float]      # относительные летучести (безразмерные)


@dataclass
class SectionProps:
    """Физические свойства фаз в сечении колонны (верх/низ). Единицы - СИ."""
    rho_liq: float    # плотность жидкости, кг/м3
    rho_vap: float    # плотность пара, кг/м3
    mu_liq: float     # ДИНАМИЧЕСКАЯ вязкость жидкости, Па*с (СИ!)
...
class MaterialBalance:
    D_flow: float           # кмоль/с
    R_flow: float            # кмоль/с
...
def material_balance(F: float, mixture: Mixture,
    ...
    """
    Простой покомпонентный материальный баланс колонны.

    F           - расход питания, КМОЛЬ/С (СИ)
    recovery_D  - доля извлечения каждого компонента в дистиллят
                  (в том же порядке, что names / xF)

    Возвращает потоки D и R также в кмоль/с.
    """
