...
vapor_ejector = VaporEjector(active, passive, common)
...
p("Энтропия на нижней пограничной кривой s(a)':",
  f"{vapor_ejector.entropy_lower_boundary / KCAL_TO_J:.2f}", "ккал/(кг·°С)")
p("Энтропия на верхней пограничной кривой s(a)'':",
  f"{vapor_ejector.entropy_upper_boundary / KCAL_TO_J:.2f}", "ккал/(кг·°С)")
p("Энтальпия на нижней пограничной кривой i(a)':",
  f"{vapor_ejector.enthalpy_lower_boundary / KCAL_TO_J:.2f}", "ккал/кг")
...
p("Температура в критическом сечении сопла pкр:",
  f"{vapor_ejector.temperature_critical:.2f}", "K")
...
p("Коэффициент восстановления давления φ:",
  f"{vapor_ejector.pressure_recovery_coefficient:.2f}")
p("Число Маха M3:", f"{vapor_ejector.mach_number:.2f}")
...
for i, p3_active in enumerate(vapor_ejector.stage_partial_pressures_active, 1):
    p(f"p3_active({i}):", f"{p3_active / PA_TO_MPA:.3f}", "МПа")

_minor_divider()
print("Газовая постоянная смеси:")
for i, gas_constant_mixture in enumerate(vapor_ejector.stage_gas_constants_mixture, 1):
...
for i, (t3, q) in enumerate(
        zip(vapor_ejector.stage_temperatures_cyl_exit,
            vapor_ejector.stage_entrainment_ratios), 1):
    p(f"T3({i}):",
...
p("Степень расширения сопла ϑ:",
  f"{vapor_ejector.nozzle_exit_area / vapor_ejector.nozzle_exit_area_theoretical:.2f}")
