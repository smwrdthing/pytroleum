...
# NOTE насколько нам нужен тип HydrocycloneType? На мой взгляд делать ветвление
# NOTE в расчётных функциях в зависимости от передаваемой строки - не очень практично
# NOTE
# NOTE 1. Сами функции становятся больше, их труднее будет тестировать, слишком много
# NOTE    нагрузки на одну функцию, модульность, расширяемость и переисопльзуемость
# NOTE    страдают
# NOTE
# NOTE 2. При использовании функции можно неправильно передать ключ-строку
# NOTE    (опечатка, например). Если не писать в каждой функции проверку такого сценария
# NOTE    - мы тихо выйдем из функции без результатов расчёта, а дальше может произойти
# NOTE    что угодно
# NOTE    (скорее всего где-то что-то отвалится, потому что вместо данных придёт None)
# NOTE
# NOTE Здесь лучше будет смотреться ООП с абстрактным базовым классом под гидроциклоны
# NOTE для разделения твёрдых частиц и жидкостей, дальеш за счёт наследования можно писать
# NOTE код для конкретных моделей
# NOTE
# NOTE По асбтрактным классам можно посмотреть/почитать на Хабре, RealPython или в
# NOTE официальной документации самого Python, пример использования в коде для
# NOTE гидроциклоно жидкость-жидкость
...
class GeometryParameters:
    ...
    # NOTE можно переработать датакласс так, чтобы доступ к конкретным
    # NOTE размерам гидроциклона был проще
    # NOTE
    # NOTE Сейчас нужно запоминать, что есть что (Dc, Di, Do и так далее).
    # NOTE При правильном подходе эту ментальную нагрузку можно снять с человека
    # NOTE и переложить на компьютер, диаметры следует организовать в какую-нибудь
    # NOTE структуру данных (список, например), для индексов можно завести константы
    # NOTE (самый простой способ) или воспользоваться Enum
    # NOTE (это чуть сложнее, пример в коде по гидроциклонам жидкость-жидкость)
    ...
    def check_proportions(self) -> List[str]:
        ...
        # NOTE 0.14, 0.28, 0.20 и т.д. здесь так-называемые "магические числа"
        # NOTE cм. https://ru.wikipedia.org/wiki/Магическое_число_(программирование)
        # NOTE так как здесь диапазонов много, то это может быть оправдано, если эти
        # NOTE значения больше нигде не используются
        # NOTE (то же верно для численных коэффициентов в корреляциях)
        # NOTE
        # NOTE В общем случае от такого лучше уходить, для чего заводят "константы" -
        # NOTE именованные переменные, которые хранят значения. В Python нет настоящих
        # NOTE констант, поэтому постоянные значения конвенционально именуются капсом
        # NOTE (THIS_IS_CONSTANT_VALUE = ...)
    ...
    def get_proportions(self) -> Dict[str, float]:
        ...
        return {
            ...
            'L/Dc': self.L / self.Dc,
            'θ': self.theta
            # NOTE угол -  не пропорция, следует переименовтаь функцию, чтобы она была
            # NOTE "честнее"
        }
...
# NOTE В отношении OperatingParameters тоже следует задуматься о структурных изменениях.
# NOTE Объявление Cv в обоих датаклассах нарушает DRY (Don't Repeat Yourself). Тут
# NOTE некритично, потому что повторяющаяся часть не очень большая
# NOTE
# NOTE Второй момент - не очень практично заводить целый dataclass для объекта с двумя
# NOTE полями
# NOTE
# NOTE В целом есть возможности разрешить два сценария с разными входными данными чуть
# NOTE аккуратнее
# NOTE
# NOTE Я бы делал разные методы в классах для гидроциклонов, разные методы -
# NOTE разные сигнатуры - нет необходимости заводить датаклассы
...
# NOTE типы объявлять сверху, после констант (если они дейстивтельно нужны)
...
def _euler_number(
    geometry: GeometryParameters,
    Cv: float,
    # NOTE говорящие имена переменных (что такое Cv?)
    # NOTE хоть это и есть в комментарии к датаклассу, на практике будет неудобно
    # NOTE каждый раз залезать в его определение и смотреть этот комментарий.
    # NOTE Проблема уходит, если параметр сразу называется "solids_concentration"
    Re: float,
    # NOTE Re допустимо оставить, т.к. такое обозначение очень распространено в
    # NOTE литературе, то есть узнаваемо, но в целом всегда лучше стремиться к большей
    # NOTE подробности (в разумных пределах, если имя переменной занимает половину экрана
    # NOTE - лучше придумать покороче)
) -> float:
    """
...
def calculate_hydrocyclone_parameters(
    ...
    # NOTE элементы возвращаемого словаря тоже можно (и следует) аннотировать,
    # NOTE особенно если внутри какой-то сложный объект, делается так
    # NOTE
    # NOTE -> dict[KeyType, ValueType]
    """
    ...
    match operating:
        # NOTE проверка выглядит очень уж странно, я бы ожидал здесь скорее что-то вроде
        # NOTE isinstance(operating, OperatingParametersQ)
        # NOTE (с isinstance следует соблюдать осторожность, иногда работает странно)
        # NOTE
        # NOTE то же для проверки ниже
        # NOTE
        # NOTE Если делать через ООП с разными методами в таких проверках в целом
        # NOTE пропадает необходимость
    ...
    # Водное отношение Rw # NOTE перевод ?
...
# NOTE говорящие имена переменных для функций ниже
...
def calculate_reduced_grade_efficiency(
    ...
    m: float,      # NOTE значения по умолячанию из литературных
    alpha: float,  # NOTE рекомендаций
) -> np.ndarray:
    """
...
def _plot_row(
    ...
    results: Dict,  # NOTE см. выше про аннотации словарей
    d: np.ndarray,
    k: float,
    n: float,
...
def plot_contour_graphs():
    # NOTE функция не достаточно общая для модуля, код отсюда лучше перенести в
    # NOTE какой-нибудь файл в директории с примерами в качестве обычного скрипта
    ...
    for row, config in enumerate(configs):
        ...
        for i in range(50):
            ...
            # NOTE убрать нативные python-циклы, использовать numpy
            # NOTE (переработать функции, если требуется)
            # NOTE
            # NOTE Python - интерпертируемый язык программирования, в первую очередь это
            # NOTE означает, что он достаточно медленный. Любой цикл, написанный на чистом
            # NOTE Python - потенциальное "бутылочное горлышко" программы. Для вычислений
            # NOTE в Python следует всячески избегать чистых циклов и использовать
            # NOTE возможности numpy.
            # NOTE
            # NOTE Numpy написан на C и предоставляет доступ к массивам
            # NOTE и операциям над ними, высоконагруженные вычисления с массивами
            # NOTE практически всегда оказываются намного быстрее Python-циклов
        ...
        ax = axes[row, 0]
        # NOTE под индексы столбцов тоже можно завести переменные (см. заметку ниже)
        contour = ax.contourf(
        ...
        if row == 0:
            # NOTE здесь 0 - точно "магическое число", для которого следует завести
            # NOTE перменную, иначе нужно помнить, что первая ось для отсечного размера
            # NOTE проверка if "row == CUT_SIZE_ROW" будет намного понятнее через время
        ...
        if row == 0:
            # NOTE то же самое
            ax.set_title('Водное отношение')  # NOTE перевод?
        ax.grid(True, alpha=0.3)
...
# NOTE Залитые контурные графики оказываются не очень репрезентативными
# NOTE (почти вся область заливается одним оттенком)
# NOTE
# NOTE Попробуй обычные конутры с подписанными линиями
# NOTE см. документацию matplotlib, например :
# NOTE https://matplotlib.org/stable/gallery/images_contours_and_fields/contour_demo.html


# NOTE какова в итоге постановка задачи/процедура проектного/поверочного расчёта для
# NOTE такого гидроциклона? Как мы можем воспользоваться уравнениями для определения
# NOTE размеров или рабочего диапазона параметров?
